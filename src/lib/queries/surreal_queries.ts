...
export type CreateUserVariables = {    referrer?: RecordId< |'user'>,    last_name: string,    calling_code: string,    phone_number: string,    password: string,    email: string,    country_code: string,    first_name: string,}
...
export type UserLoginVariables = {    password: string,    email: string,}
export const AddStatementQuery = "CREATE statement CONTENT $statement;"
export type AddStatementResult = [Array<{author:RecordId< |'user'>,content:string,created_at:Date,id:RecordId< |'statement'>,replying_to:RecordId< |'statement'>|undefined,side:string|undefined,updated_at:Date,}>,]
export type Queries = {
    ...
    [AddStatementQuery]: {variables: never, result: AddStatementResult }
}
